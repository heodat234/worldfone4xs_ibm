...
try:
   ...
   for x in range(0,3):
      ...
      month             = dateAddMonths.month
      year              = dateAddMonths.year

      todayString       = dateAddMonths.strftime("%d/%m/%Y")
      todayTimeStamp    = int(time.mktime(time.strptime(str(todayString + " 00:00:00"), "%d/%m/%Y %H:%M:%S")))

      day_of_month      = monthrange(year, month)[1]
      end_day_of_month  = dateAddMonths.replace(day=day_of_month)
      ...
      start_day_of_month   = dateAddMonths.replace(day=1)
      ...
      # List cmnd >361
      aggregate_pipeline = [
         { "$match" : {'account_number': {'$nin': account_sibs_for_month} } },
      ...
      # List cmnd >361
      aggregate_pipeline = [
         { "$match" : {'account_number': {'$nin': account_card_for_month} } },
