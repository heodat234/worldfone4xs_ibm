	...
	for i in range(count):	
		...
		if results_outsoucing != None:
			for row_result in results_outsoucing:
				...
				subdate = firstDayOfMonth - due_date	
				get_ln3206f = mongodb.getOne(MONGO_COLLECTION=collection_ln3206f, WHERE={'account_number':account_number, 'code':'10'})
				plus = 0
				amount = 0
				if get_ln3206f != None:
					plus = 1
					...
					get_gl_2018 = mongodb.get(MONGO_COLLECTION=collection_gl_2018, WHERE={'$and' : [ {'account_number':account_number}, {'$or' : [{'code': '2000'}, {'code': '2100'}, {'code': '2700'}]}]})
					if get_ln3206f != None:
						code_2000 = False
						code_2100 = False
						code_2700 = False
				...
				where = {'partner':row_result['COMPANY'], 'year' : {'$exists' : True, '$eq' : str(year)} }
				if(subdate.days<30):
					modified = mongodb.inc(MONGO_COLLECTION=collection_amount_report,WHERE=where,VALUE={'collected.account.before.l30.T'+str(month) :plus, 'collected.amount.before.l30.T'+str(month) : amount})
				elif(subdate.days>=30 and subdate.days < 60):
					modified = mongodb.inc(MONGO_COLLECTION=collection_amount_report,WHERE=where,VALUE={'collected.account.before.p30.T'+str(month) :plus, 'collected.amount.before.p30.T'+str(month) : amount})
				elif(subdate.days>=60 and subdate.days < 90):
					modified = mongodb.inc(MONGO_COLLECTION=collection_amount_report,WHERE=where,VALUE={'collected.account.before.p60.T'+str(month) :plus, 'collected.amount.before.p60.T'+str(month) : amount})
				elif(subdate.days>=90 and subdate.days < 180):
					modified = mongodb.inc(MONGO_COLLECTION=collection_amount_report,WHERE=where,VALUE={'collected.account.before.p90.T'+str(month) :plus, 'collected.amount.before.p90.T'+str(month) : amount})
				elif(subdate.days>=180 and subdate.days < 360):
					modified = mongodb.inc(MONGO_COLLECTION=collection_amount_report,WHERE=where,VALUE={'collected.account.before.p180.T'+str(month):plus, 'collected.amount.before.p180.T'+str(month) : amount})
				else: continue
			...
			if results_outsoucing != None:
				for row_result in results_outsoucing:
					...
					if(row_result['DPD'] == '<30'):
						modified = mongodb.inc(MONGO_COLLECTION=collection_assigned_report,WHERE=where,VALUE={'outsoucing.account.before.l30.T'+str(month) :1, 'outsoucing.amount.before.l30.T'+str(month) : row_result['CURRENT_DEBT']})
					elif(row_result['DPD'] == '30+'):
						modified = mongodb.inc(MONGO_COLLECTION=collection_assigned_report,WHERE=where,VALUE={'outsoucing.account.before.p30.T'+str(month) :1, 'outsoucing.amount.before.p30.T'+str(month) : row_result['CURRENT_DEBT']})
					elif(row_result['DPD'] == '60+'):
						modified = mongodb.inc(MONGO_COLLECTION=collection_assigned_report,WHERE=where,VALUE={'outsoucing.account.before.p60.T'+str(month) :1, 'outsoucing.amount.before.p60.T'+str(month) : row_result['CURRENT_DEBT']})
					elif(row_result['DPD'] == '90+'):
						modified = mongodb.inc(MONGO_COLLECTION=collection_assigned_report,WHERE=where,VALUE={'outsoucing.account.before.p90.T'+str(month) :1, 'outsoucing.amount.before.p90.T'+str(month) : row_result['CURRENT_DEBT']})
					elif(row_result['DPD'] == '180+'):
						modified = mongodb.inc(MONGO_COLLECTION=collection_assigned_report,WHERE=where,VALUE={'outsoucing.account.before.p180.T'+str(month):1, 'outsoucing.amount.before.p180.T'+str(month) : row_result['CURRENT_DEBT']})
					else: continue
	#2.COLLECTED
	pipeline_count = [
		{
	...
	for i in range(count):	
		...
		if results_outsoucing != None:
			for row_result in results_outsoucing:
					...
					where = {'partner':row_result['COMPANY'], 'year' : {'$exists' : True, '$eq' : str(year)} }
					if(row_result['DPD'] == '<30'):
						modified = mongodb.inc(MONGO_COLLECTION=collection_assigned_report,WHERE=where,VALUE={'collected.account.before.l30.T'+str(month) :plus, 'collected.amount.before.l30.T'+str(month) : amount})
					elif(row_result['DPD'] == '30+'):
						modified = mongodb.inc(MONGO_COLLECTION=collection_assigned_report,WHERE=where,VALUE={'collected.account.before.p30.T'+str(month) :plus, 'collected.amount.before.p30.T'+str(month) : amount})
					elif(row_result['DPD'] == '60+'):
						modified = mongodb.inc(MONGO_COLLECTION=collection_assigned_report,WHERE=where,VALUE={'collected.account.before.p60.T'+str(month) :plus, 'collected.amount.before.p60.T'+str(month) : amount})
					elif(row_result['DPD'] == '90+'):
						modified = mongodb.inc(MONGO_COLLECTION=collection_assigned_report,WHERE=where,VALUE={'collected.account.before.p90.T'+str(month) :plus, 'collected.amount.before.p90.T'+str(month) : amount})
					elif(row_result['DPD'] == '180+'):
						modified = mongodb.inc(MONGO_COLLECTION=collection_assigned_report,WHERE=where,VALUE={'collected.account.before.p180.T'+str(month):plus, 'collected.amount.before.p180.T'+str(month) : amount})
					else: continue	
