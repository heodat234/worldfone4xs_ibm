#!/usr/bin/python3.6
# -*- coding: utf-8 -*-
log = open("/var/www/html/worldfone4xs_ibm/cronjob/python/Loan/log/importReportInputPaymentOfCard.txt","a")
...
try:
    filename = 'gl_20180919_REPORT INPUT PAYMENT OF CARD.csv'
    filepath = '/var/www/html/worldfone4xs_ibm/upload/csv/ftp/'
    collection = 'LO_Report_input_payment_of_card'
    collectionResult = 'LO_Report_input_payment_of_card_result'
    ...
    if(filenameExtension[1] == 'csv'):
        inputDataRaw = excel.getDataCSV(file_path=importLogInfo['file_path'], header=None, names=modelColumns, na_values='')
    else:
            ...
            try:
                if(modelConverters[cell] == 'timestamp'):
                    if row[cell] is not None and row[cell] is not '' and row[cell] is not ' ':
                        temp[cell] = common.convertDataType(data=row[cell], datatype=modelConverters[cell], formatType="%y%m%d")
                else:
