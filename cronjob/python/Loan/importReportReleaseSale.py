...
try:
    ...
    month = today.month
    year = today.year
    fileName = "ReportReleaseSale.xlsx"
    sep = ';'
    ...
    try:
        ...
        importLogInfo = {
            ...
            'source'        : 'ftp',
            'status'        : 2,
            'command'       : '/usr/local/bin/python3.6 ' + base_url + "cronjob/python/Loan/importReportReleaseSale.py > /dev/null &",
            'created_by'    : 'system'
        }
    ...
    if filenameExtension[1] in ['csv', 'xlsx']:
        if(filenameExtension[1] == 'csv'):
            ...
            inputDataRaw = excel.getDataExcel(file_path=importLogInfo['file_path'], active_sheet="Sheet1", header=None, names=modelColumns, na_values='')
    ...
    if(len(errorData) > 0):
        mongodbresult.remove_document(MONGO_COLLECTION=common.getSubUser(subUserType, ('Report_release_sale_' + str(year) + str(month) + str(day))))
        mongodbresult.batch_insert(common.getSubUser(subUserType, ('Report_release_sale_' + str(year) + str(month) + str(day))), errorData)
