...
try:
    ...
    # today = date.today()
    today = datetime.strptime('14/12/2019', "%d/%m/%Y").date()
                    ...
                    for key in dpWorkingdaysdaycol:
                        ...
                        if len(groupInfoByDueDate) > 0:
                            ...
                            for groupCell in groupInfoByDueDate:
                                ...
                                groupCell['name'] = groupCell['name'].replace('/G1', '')
                                if groupCell['name'] != 'Total':
                                    yesterdayInfo = mongodb.getOne(MONGO_COLLECTION=collection, WHERE={'$or': [{'updated_at': {'$gte': yesterday_starttime, '$lte': yesterday_endtime}}, {'created_at': {'$gte': yesterday_starttime, '$lte': yesterday_endtime}}], 'team_id': str(groupCell['_id'])})
                                    debtList = []
                                    cur_bal = 0
                                    count_acc = 0
                                    ...
                                    paid_acc_end_date_yesterday = yesterdayInfo['index_' + str(todayIndex - 1)] - no_overdue if yesterdayInfo is not None and 'index_' + str(todayIndex - 1) in yesterdayInfo.keys() is not None else 0
                                    temp = {}
                                    if key == '1':
                                    ...
                                    if key == '2':
                                        temp['index_' + str(todayIndex - 1)] = yesterdayInfo['index_' + str(todayIndex - 1)] - no_overdue if yesterdayInfo is not None and 'index_' + str(todayIndex - 1) in yesterdayInfo.keys() is not None else 0
                                        temp['index_' + str(todayIndex)] = 0
                                        total_no_paid_acc_end_day += temp['index_' + str(todayIndex - 1)]

                                    if key == '3':
                                        temp['index_' + str(todayIndex - 1)] = yesterdayInfo['start_acc'] - no_overdue if yesterdayInfo is not None and 'start_acc' in yesterdayInfo.keys() else 0
                                        temp['index_' + str(todayIndex)] = 0
                                    ...
                                    if key == '4':
                                        temp['index_' + str(todayIndex)] = (yesterdayInfo['index_' + str(todayIndex - 1)] - no_overdue) / yesterdayInfo['start_acc'] if yesterdayInfo is not None and 'start_acc' in yesterdayInfo.keys() is not None and 'index_' + str(todayIndex - 1) in yesterdayInfo.keys() and yesterdayInfo['start_acc'] != 0 else 0
                                    ...
                                    if key == '6':
                                        temp['index_' + str(todayIndex)] = yesterdayInfo['index_' + str(todayIndex - 1)] - no_overdue_amt if yesterdayInfo is not None and 'index_' + str(todayIndex - 1) in  yesterdayInfo.keys() is not None else 0
                                        total_collected_amt_and_day += temp['index_' + str(todayIndex)]

                                    if key == '7':
                                        temp['index_' + str(todayIndex)] = yesterdayInfo['start_amt'] - no_overdue_amt if yesterdayInfo is not None and 'start_amt' in yesterdayInfo.keys() is not None else 0
                                        total_collected_amt_accumulated += temp['index_' + str(todayIndex)]

                                    if key == '8':
                                        temp['index_' + str(todayIndex)] = (yesterdayInfo['index_' + str(todayIndex - 1)] - no_overdue_amt) / yesterdayInfo['start_amt'] if yesterdayInfo is not None and 'start_amt' in yesterdayInfo.keys() is not None and 'index_' + str(todayIndex - 1) in yesterdayInfo.keys() is not None and yesterdayInfo['start_amt'] != 0 else 0
                                        total_collected_ratio_amt += temp['index_' + str(todayIndex)]

                                    if yesterdayInfo is not None:
                                        mongodb.update(MONGO_COLLECTION=collection, WHERE={'team_id': str(groupCell['_id']), 'day_code': key, '$or': [{'updated_at': {'$gte': yesterday_starttime, '$lte': yesterday_endtime}}, {'created_at': {'$gte': yesterday_starttime, '$lte': yesterday_endtime}}]}, VALUE=temp)
                                    else:
                                    ...
                                    #     pprint(groupCell)
                                    mongodb.update(MONGO_COLLECTION=collection, WHERE={'name': 'Total', 'day_code': key, 'group': temp['group'], 'month': temp['month'], 'due': temp['due'], 'product': temp['product'], '$or': [{'updated_at': {'$gte': yesterday_starttime, '$lte': yesterday_endtime}}, {'created_at': {'$gte': yesterday_starttime, '$lte': yesterday_endtime}}]}, VALUE=temp)
    print('DONE')
