...
log = open(base_url + "cronjob/python/Loan/log/importThuHoiXe.txt","a")
now = datetime.now()
subUserType = 'LO'
...
try:
    ...
    month = today.month
    year = today.year
    fileName = "DANH SACH THU HOI XE.xlsx"
    sep = ';'
    ...
    try:
        ...
        importLogInfo = {
            ...
            'source'        : 'ftp',
            'status'        : 2,
            'command'       : '/usr/local/bin/python3.6 ' + base_url + "cronjob/python/Loan/importThuHoiXe.py > /dev/null &",
            'created_by'    : 'system'
        }
    ...
    if(len(errorData) > 0):
        mongodbresult.remove_document(MONGO_COLLECTION=common.getSubUser(subUserType, ('Thu_hoi_xe_' + str(year) + str(month) + str(day))))
        mongodbresult.batch_insert(common.getSubUser(subUserType, ('Thu_hoi_xe_' + str(year) + str(month) + str(day))), errorData)
