...
log = open(base_url + "cronjob/python/Loan/log/saveDailyProductivityReport.txt","a")
now = datetime.now()
subUserType = 'LO'
collection = common.getSubUser(subUserType, 'Diallist')
...
try:
    ...
    checkData = mongodb.get("LO_Diallist")
    print(checkData)

    # todayString = time.strftime("%d/%m/%Y")
    todayString = '13/02/2020'
    todayTimeStamp = int(time.mktime(time.strptime(str(todayString + " 00:00:00"), "%d/%m/%Y %H:%M:%S")))
    print(todayTimeStamp)

    lastDayOfMonth = calendar.monthrange(year, month)
