...
log = open(base_url + "cronjob/python/Loan/log/importCollateralInfo.txt","a")
now = datetime.now()
subUserType = 'LO'
collection = common.getSubUser(subUserType, 'Collateral_info')
...
try:
    ...
    try:
        ...
        # ftp.downLoadFile(serverfolder='/testFTP', filename=ftpInfo['filename'], local_path=ftpLocalUrl)
    ...
    if(len(errorData) > 0):
        mongodb.batch_insert(common.getSubUser(subUserType, 'Collateral_info_result'), errorData)
        ...
        if len(insertData) > 0:
            ...
            mongodb.batch_insert(common.getSubUser(subUserType, 'Collateral_info_result'), insert_data=insertData)
