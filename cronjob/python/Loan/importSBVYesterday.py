...
try:
    ...
    log = open(base_url + "cronjob/python/Loan/log/importSBVYesterday.txt","a")
    now = datetime.now()
    subUserType = 'LO'
    collection = common.getSubUser(subUserType, 'SBV_yesterday')
    collectionModel = common.getSubUser(subUserType, 'SBV')
    ...
    thedaybefore = yesterday - timedelta(days=1)
    day = today.day
    month = today.month
    year = today.year
    todayString = today.strftime("%d/%m/%Y")
    fileName = "sbv_" + thedaybefore.strftime("%Y%m%d") + ".dat"
    sep = ';'
    ...
    try:
        ...
        importLogInfo = {
            ...
            'source'        : 'ftp',
            'status'        : 2,
            'command'       : '/usr/local/bin/python3.6 ' + base_url + "cronjob/python/Loan/importSBVYesterday.py > /dev/null &",
            'created_by'    : 'system'
        }
        importLogId = mongodb.insert(MONGO_COLLECTION=common.getSubUser(subUserType, 'Import'), insert_data=importLogInfo)

    models = _mongodb.get(MONGO_COLLECTION='Model', WHERE={'collection': collectionModel}, SORT=[('index', 1)], SELECT=['index', 'collection', 'field', 'type', 'sub_type'], TAKE=1000)
    ...
    if filenameExtension[1] in ['csv', 'xlsx']:
        ...
        with open(importLogInfo['file_path'], 'r', newline='\n', encoding='ISO-8859-1') as fin:
            ...
            for idx, row in enumerate(csv_reader):
                ...
                result = True
                temp = {}
                if isinstance(row[0], str) and len(row[1]) > 12 and row[0].isdigit():
    ...
    if(len(errorData) > 0):
        mongodbresult.remove_document(MONGO_COLLECTION=common.getSubUser(subUserType, ('SBV_' + str(year) + str(month) + str(day))))
        mongodbresult.batch_insert(common.getSubUser(subUserType, ('SBV' + str(year) + str(month) + str(day))), errorData)
