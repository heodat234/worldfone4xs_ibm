...
try:
    ...
    collectionModel = common.getSubUser(subUserType, 'List_of_account_in_collection')
    ...
    month = today.month
    year = today.year
    fileName = "LIST_OF_ACCOUNT_IN_COLLECTION_" + thedaybefore.strftime("%Y%m%d")
    sep = ','
    ...
    models = _mongodb.get(MONGO_COLLECTION='Model', WHERE={'collection': collectionModel, 'sub_type': {'$ne': None}}, SORT=[('index', 1)], SELECT=['index', 'collection', 'field', 'type', 'sub_type'], TAKE=1000)
    for model in models:
