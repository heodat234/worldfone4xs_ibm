...
try:
    ...
    try:
        ...
        importLogInfo = {
            ...
            'source'        : 'ftp',
            'status'        : 2,
            'command'       : 'python3.6 ' + base_url + "cronjob/python/Loan/importInvestigationFile.py > /dev/null &",
            'created_by'    : 'system'
        }
    ...
    if(len(errorData) > 0):
        mongodb.batch_insert(common.getSubUser(subUserType, 'Investigation_file_result'), errorData)
        ...
        if len(insertData) > 0:
            ...
            mongodb.batch_insert(common.getSubUser(subUserType, 'Investigation_file_result'), insert_data=insertData)
