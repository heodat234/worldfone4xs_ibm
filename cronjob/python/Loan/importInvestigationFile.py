...
try:
    ...
    log = open(base_url + "cronjob/python/Loan/log/importInvestigationFile.txt","a")
    now = datetime.now()
    subUserType = 'LO'
    collection = common.getSubUser(subUserType, 'Investigation_file')
    ...
    complete = 0
    today = date.today()
    # today = datetime.strptime('23/12/2019', "%d/%m/%Y").date()
    day = today.day
    month = today.month
    year = today.year
    fileName = "Investigation file.xlsx"
    sep = ';'
    ...
    try:
        ...
        if not os.path.isfile(ftpLocalUrl):
            notification = {
                'title'     : 'Import INVESTIGATION FILE error',
                'active'    : True,
                'icon'      : 'fa fa-exclamation-triangle',
                'color'     : 'text-warning',
                'content'   : 'Không có file INVESTIGATION FILE hôm nay',
        ...
        importLogInfo = {
            ...
            'source'        : 'ftp',
            'status'        : 2,
            'command'       : '/usr/local/bin/python3.6 ' + base_url + "cronjob/python/Loan/importInvestigation.py > /dev/null &",
            'created_by'    : 'system'
        }
    ...
    if(len(errorData) > 0):
        mongodbresult.remove_document(MONGO_COLLECTION=common.getSubUser(subUserType, ('Investigation_file_' + str(year) + str(month) + str(day))))
        mongodbresult.batch_insert(common.getSubUser(subUserType, ('Investigation_file_' + str(year) + str(month) + str(day))), errorData)
