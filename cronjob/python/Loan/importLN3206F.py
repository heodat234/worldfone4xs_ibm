#!/usr/bin/python3.6
# -*- coding: utf-8 -*-
log = open("/var/www/html/worldfone4xs_ibm/cronjob/python/Loan/log/importln3206f.txt","a")
...
try:
    filename = 'LN3206F.csv'
    filepath = '/var/www/html/worldfone4xs_ibm/upload/csv/ftp/'
    collection = 'LO_LN3206F'
    collectionResult = 'LO_LN3206F_result'
    ...
    if(filenameExtension[1] == 'csv'):
        inputDataRaw = excel.getDataCSV(file_path=importLogInfo['file_path'], sep=';', header=None, names=modelColumns, na_values='', usecols=[1, 3, 4, 8, 11, 12])
    else:
        inputDataRaw = excel.getDataExcel(file_path=importLogInfo['file_path'], header=None, names=modelColumns, na_values='', usecols=[1, 3, 4, 8, 11, 12])
