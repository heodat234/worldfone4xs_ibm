...
try:
    ...
    month = today.month
    year = today.year
    fileName = "Danh sach quay tu van.xlsx"
    sep = ','
    ...
    try:
        ...
        importLogInfo = {
            ...
            'source'        : 'ftp',
            'status'        : 2,
            'command'       : 'python3.6 ' + base_url + "cronjob/python/Loan/importDealer_cron.py > /dev/null &",
            'created_by'    : 'system'
        }
    ...
    if filenameExtension[1] in ['csv', 'xlsx']:
        ...
        for idx, row in enumerate(inputData):
            temp = {}
            result = True
            if row['dealer_code'] not in ['', None] and row['dealer_name'] not in ['', None] and row['location'] not in ['', None]:
                for cell in row:
                    try:
    ...
    if(len(errorData) > 0):
        mongodbresult.remove_document(MONGO_COLLECTION=common.getSubUser(subUserType, ('Dealer_' + str(year) + str(month) + str(day))))
        mongodbresult.batch_insert(common.getSubUser(subUserType, ('Dealer_' + str(year) + str(month) + str(day))), errorData)
