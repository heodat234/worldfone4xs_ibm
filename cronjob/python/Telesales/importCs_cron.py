...
log = open(base_url + "cronjob/python/Telesales/log/importSc.txt","a")
now = datetime.now()
subUserType = 'TS'
...
try:
    ...
    try:
        ...
        importLogInfo = {
            ...
            'source'        : 'ftp',
            'status'        : 2,
            'command'       : 'python3.6 ' + base_url + "cronjob/python/Loan/importCs_cron.py > /dev/null &",
            'created_by'    : 'system'
        }
    ...
    if filenameExtension[1] in ['csv', 'xlsx']:
        ...
        for idx, row in enumerate(inputData):
            temp = {}
            result = True
            if row['sc_code'] not in ['', None] and row['sc_name'] not in ['', None]:
                for cell in row:
                    try:
