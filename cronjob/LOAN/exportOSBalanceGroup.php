...
$collection = 'LO_Os_balance_group_report';

$today      = date('Y-m-d');
$last6Month = strtotime(date("Y-m-d", strtotime($today)) . " -6 month");

$request = array('createdAt' => array('$gte' => $last6Month), 'type' => 'SIBS');
$data = $mongo_db->where($request)->order_by(array('debt_group' => 'asc', 'year' => 'asc','for_month' => 'asc','day' => 'asc'))->get($collection);

$request = array('createdAt' => array('$gte' => $last6Month), 'type' => 'CARD');
$dataCard = $mongo_db->where($request)->order_by(array('debt_group' => 'asc', 'year' => 'asc','for_month' => 'asc','day' => 'asc'))->get($collection);
...
$month_column   = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex($start_col);

$rowGroup = $borderThick = array();
$randomColor = 1;

foreach ($data as $key => $value) {
    $column         = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex($start_col);
    $next_column    = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex($start_col+1);
    ...
    if ($value['for_month'] != $for_month && $value['debt_group'] == $fix_debt_group) {
        ...
        $endMonthColumn = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex($start_col-1);
    ...
    if ($key == (count($data) - 1)) {
        ...
        $endMonthColumn = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex($start_col);
        $worksheet->mergeCells($month_column."1:".$endMonthColumn."1");
        $worksheet->setCellValue($month_column."1", $getDate['month']);
        $worksheet->getStyle($month_column."1")->getFill()
        ->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)
    ...
    $colPrincipal = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex($start_col);
    ...
    if (isset($value['check_due_date'])) {
        ...
        $colTargetTitle = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex($start_col+2);
        $colTarget      = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex($start_col+3);
    ...
    if (isset($value['final_os_bl'])) { //ngay due date, ngay final
        ...
        $column_wo_title    = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex($start_col - 4);
        $column_wo          = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex($start_col - 3);
    ...
    if ($getValueDate['wday'] == 0 || $getValueDate['wday'] == 6) {
        $column_weekday         = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex($start_col);
...
$maxCol = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex($maxCol -1);
...
foreach ($borderThick as $value) {
    if ($value['row'] == 1) {
        ...
        $worksheet->getStyle($value['col'].($value['row']+1).":".$value['col'].($value['row']+16))->getBorders()
        ->getRight()->setBorderStyle(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THICK);
    }
}
...
foreach ($borderThick as $value) {
    if ($value['row'] == 1) {
        ...
        //to dam border cuoi ki due
        $worksheet->getStyle($value['col'].($value['row']+1).":".$value['col'].($value['row']+14))->getBorders()
        ->getRight()->setBorderStyle(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THICK);
    }
}
$worksheet->getStyle("B1:B".$maxCell['row'])->getBorders()
...
$file_path = "/var/www/html/worldfone4xs_ibm/upload/loan/export/" . 'DailyReportOfOSBlanceOfGroupBCDE.xlsx';
