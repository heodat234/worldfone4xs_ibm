...
//$inputFileName = rtrim($app_path, "/") . "/cronjob/LOAN/LNJC05F";
$folder = "20191120";
$inputFileName = "/data/upload_file/{$folder}/LNJC05F";
...
$model = $mongo_db->where(array("collection" => "LO_LNJC05"))->like("sub_type", "import")->order_by(array("index" => 1))->get("Model");

$fieldToType = [];
foreach ($model as $doc) {
    if(isset($doc["field"]) && !in_array($doc["field"], $header)) {
        $header[] = $doc["field"];
        $fieldToType[$doc["field"]] = isset($doc["type"]) ? $doc["type"] : "string";
    }
}
...
if(!in_array($collection, $exists_collections)) 
{
    ...
    $index_arr = [$key_field => -1];
    if(isset($key_field_2)) $index_arr[$key_field_2] = -1;
...
while(!feof($file))
{
    ...
    if(!empty($temp)) {
        ...
        if(empty($editedValue[2])) continue;
        ...
        if($header) {
            foreach ($header as $index => $field) {
                switch ($field) {
                    case 'due_date':
                        $doc[$field] = convertStringToTimestamp($editedValue[$index]);
                        break;
                    
                    default:
                        switch ($fieldToType[$field]) {
                            case 'int':
                                $doc[$field] = isset($editedValue[$index]) ? (int) $editedValue[$index] : 0;
                                break;
        ...
        if(isset($key_field_2)) $queueData["key_field_2"] = $key_field_2;
