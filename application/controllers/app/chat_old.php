...
//$this->get_daily($user_id)
    ...
    public function loadRoom($type = '') {
        ...
        foreach ($results as $value) {
            ...
            if ($value['source'] == "messenger") {
                ...
                // $icons = '<img src="'. base_url() . '"assets/images/message_icon.svg"/>';
                // $icons = base_url() . 'assets/images/zalo_favicon.ico';
                // $icon = base_url().'assets/images/flogo_rgb_hex-brc-site-250.png';
                $icons = array(
                ...
                // $line = base_url().'img/if_Facebook_comment.png';
                ...
                $icons = array(
                    ...
                    base_url() . 'assets/images/message_icon.svg',
                );//= '<img src="'. base_url() . '"assets/images/message_icon.svg"/>';
                //$icon = base_url() . 'assets/images/message_icon.svg';
            } elseif ($value['source'] == "viber") {
                $icons = base_url() . 'img/if_Viber_3030.png';
            } else {
                $icons = array();
            }
            ...
            if (isset($value['page_id'])) {
                if ($value['source'] == 'messenger' || $value['source'] == 'facebook') {
                  ...
                  $page_info = $this->mongo_db->where( array("page_id" => $value['page_id'] ))->getOne('zalo_page');
                  $page_name = $page_info['page_info']['name'];
                  }elseif ($value['source'] == 'viber') {
                  $page_info = $this->mongo_db->where( array("public_account_id" => $value['page_id'] ))->getOne('viber_page');
                  $page_name = $page_info['page_info']['name'];
                 */
            }
    ...
    public function getNameRoom($room_id) {
        ...
        if (empty($people_info_pp)) {
            ...
            $result2 = $this->mongo_db->insert('people', $data_peple);
        }
...
//            $group_name = $room['from']['name'];
    ...
    public function loadChatMes($room_id, $page = 0) {
        ...
        //$people_pp = $this->mongo_db->get('people');
        // print_r($people_pp); exit();
//        end
        foreach ($results as $result) {
