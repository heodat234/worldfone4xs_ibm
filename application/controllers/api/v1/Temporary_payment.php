    ...
    function importFTP()
    {
        try {
            ...
            if(!empty($checkScriptRunning)) {
                ...
                $pythonCron = FCPATH . 'cronjob/python/Loan/importTemporaryPayment.py';
        ...
        try {
            ...
            if(!empty($checkScriptRunning)) {
                ...
                $importLog = array(
                    ...
                    'file_path'     => FCPATH . $request['filepath'],
                ...
                //     $command = escapeshellcmd('/usr/local/bin/python3.6 ' . $pythonCron . ' ' . $importLogId['id'] . ' > /dev/null &');
                // }
                $dir = '/usr/local/bin/python3.6 ' . $pythonCron . ' ' . $importLogId['id'] . ' > /dev/null &';
                $this->mongo_db->insert('LO_Cron_scanning', array('dir' => $dir, 'runned' => false));
                // $output = shell_exec($command);
                echo json_encode(array('status' => 2, "message" => "@Importing... Please check import history for more detail@"));
            }
        } catch (Exception $e) {
            ...
            /* if(!empty($response['data'])) {
                foreach($response['data'] as $key => &$value) {
                    if(!empty($value['account_number'])) {
                        $telesaleListInfo = $this->crud->where(array('account_number' => $value['account_number']))->getOne(set_sub_collection('LNJC05'));
                        $value['lnjc05'] = (!empty($telesaleListInfo)) ? $telesaleListInfo : array();
                    }
