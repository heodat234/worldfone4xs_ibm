...
Class Card extends WFF_Controller {
	...
	function read()
	{
		try {
			...
			$select = array("contract_no", 'interest_rate', 'approved_limit', 'open_card_date', 'ob_principal_sale', 'ob_principal_cash', 'expiry_date', 'delinquency_group');
			$debtGroup = array("A", "B", "C", "D", "E");
			...
			foreach ($response['data'] as $key => &$value) {
				...
				$value["debt_group"] = (!empty($debtGroup[(int)$value["delinquency_group"] - 1])) ? $debtGroup[(int)$value["delinquency_group"] - 1] : $value["delinquency_group"];
				...
				if(!empty($listofacc)){
					$value['due_date'] 						= date('d/m/Y', $listofacc["due_date"]);
					$value['no_of_overdue_date'] 			= floor((int)((time() - $listofacc["due_date"])) / 86400);
				...
				// $expiry_date 				= DateTime::createFromFormat('dmY', $value['expiry_date']);
