 ...
 * http://phpgainers.com/content/how-use-mongo-db-codeigniter/
 */

/**
 * @property Mongo $connection  Mongo Connection object.
 * @property MongoDB $db     MongoDB database object
 * @property WFF_Controller $CI  Instance of super object Controller.
 */

class Mongo_db4x {

    private $CI;
    private $config_file = 'mongodb4x';
    private $connection;
    private $db;
     ...
     * 	criteria.
     *
     * 	@usage : $this->mongo_db->group(array('foo' => 'bar'))->get('foobar');
     */
    ...
    public function where_exists($field = '') {
        $this->_where_init($field);
    ...
    public function find($collection = "",$where) {
        ...
        $document = $this->db->{$collection}->find($where);
     ...
     * 	Get the documents based upon the passed parameters
     *
     * 	@usage : $this->mongo_db->distinct('foo');
     * 
...
//        ini_set('mongo.native_long', 1);
        ...
        //$result = iterator_to_array($documents);
        // Clear
        $this->_clear();
     ...
     * 	--------------------------------------------------------------------------------
     *
     * 	aggregate the documents based upon the pipeline parameters
     *
     * 	@usage : $this->mongo_db->aggregate_pipeline('foo',$pipeline);
     */
...
//        ini_set('mongo.native_long', 1);
    ...
    /**
     * 	--------------------------------------------------------------------------------
     * 	// aggregate_pipeline_usedisk
     * 	--------------------------------------------------------------------------------
     *
     * 	aggregate the documents based upon the pipeline parameters. With option usedisk
     *
     * 	@usage : $this->mongo_db->aggregate('foo');
     */
    ...
    public function arrayCommand($query = array()) {
        try {
            $run = $this->db->command($query);
            $result = iterator_to_array($run);
            return $result;
        } catch (MongoCursorException $e) {
            show_error("MongoDB command failed to execute: {$e->getMessage()}", 500);
        }
