    ...
    <div class="row">
		...
		<script type="text/javascript">
			window.onload = function() {
				...
				followUpFields.read().then(function(){
					...
					columns.map(col => {
						switch (col.type) {
							case "phone":
								col.template = (dataItem) => gridPhoneDialId(dataItem[col.field], dataItem.foreign_id, "manual");
								break;
							case "arrayPhone":
			...
			function detailData(ele) {
				var uid = $(ele).data('uid');
				var dataItem = Table.dataSource.getByUid(uid);
				window.open(ENV.vApi + "redirect/fromPhoneToCustomerDetail/" + dataItem.phone,'_blank','noopener');
			}
