...
var Config = {
    ...
    collection: "telesalelist_assigning_log",
    observable: {
    },
    model: {
        id: "id"
...
<ul class="breadcrumb breadcrumb-top">
    ...
    <li class="pull-right none-breakcrumb">
        <div class="input-group-btn column-widget">
			...
			<a style="margin-left: 20px" role="button" class="btn btn-sm" onclick="Table.grid.saveAsExcel()"><i class="fa fa-file-excel-o"></i> <b>@Export@</b></a>
        </div>
    </li>
</ul>
		...
		var Config = Object.assign(Config, {
		    ...
		    parse: function(response) {
		    	response.data.map(function(doc) {
					doc.request_time = new Date(doc.request_time * 1000);
					doc.updated_at = new Date(doc.updated_at * 1000);
		            return doc;
		        })
		        return response;
		    },
			...
			window.onload = function() {
				var followUpFields = new kendo.data.DataSource({
					...
					filter: [{
						field: "collection",
						operator: "eq",
						value: (ENV.type ? ENV.type + "_" : "") + "Telesalelist_assigning_log"
					}, {
                        field: 'sub_type',
                        operator: 'isnotempty'
                    }],
