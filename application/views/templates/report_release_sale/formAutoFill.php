		...
		<div id="side-form" style="width: 50%">
			...
			<div class="form-group">
                ...
                <div style="float: right; display: block; white-space: nowrap">
                    <label>
                        <input id="isAgentAssign" style="vertical-align: middle" type="checkbox" data-bind="checked: isAgentAssign, events: {click: isAgentAssignClick}" />
                        <span style="vertical-align: middle">@Assign to agent@</span>
                    </label>
                </div>
                <br>
				<input id="assign-agent" 
                    data-role="dropdownlist"
                    data-text-field="extension"
                    data-value-field="extension"
                    data-filter="contains"
                    data-bind="value: item.assignRaw, source: assignOptionAgent, visible: isAgentAssign" style="width: 100%;"/>
                <input data-role="dropdownlist"
                       data-value-primitive="true"
                       data-text-field="name"
                       data-value-field="id"
                       data-filter="contains"
                       data-bind="value: item.assignGroup, source: assignOptionGroup, invisible: isAgentAssign" style="width: 100%"/>
			</div>
			<div class="form-group">
...
var ticketOption = <?= !empty($doc) ? json_encode($doc) : '{}' ?>;
    if(ticketOption.assign.length == 1) {
        ticketOption.assignRaw = ticketOption.assign[0];
    }
var ticketItem = Object.assign({
		status: "Open",
...
var ticketObservable = {
	isAgentAssign: (JSON.parse(ticketItem.isAgentAssign)) ? true : false,
	ci_name: '',
    ci_phone: '',
    ci_email: '',
    ci_no: '',
	...
	save: function() {
        ...
        if(typeof this.item.id !== 'undefined') {
            url = ENV.vApi + "ticket/update/" + this.item.id;
        }
        else {
            url = ENV.vApi + "ticket/create";
        }
        if(!this.item.isAgentAssign) {
            delete this.item.assignRaw;
            delete this.item.assign;
        }
        else {
            if(typeof this.item.assignGroup !== 'undefined') {
                delete this.item.assignGroup;
                delete this.item.assignGroupName;
            }
        }
        $.ajax({
            url: url,
            ...
            success: (response) => {
                if(response.status == 1) {
                    notificationAfterRefresh("@Update@ @ticket@ @success@", "success");
                    location.reload();
                }
                else{
	...
	isAgentAssignClick: function(e) {
        if($("#isAgentAssign").prop('checked')) {
            this.set('item.isAgentAssign', true);
        }
        else {
            this.set('item.isAgentAssign', false);
        }
        console.log(this.get('item.isAgentAssign'));
    },
