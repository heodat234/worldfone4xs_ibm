...
<script>
    var model = kendo.observable({
        historyData: new kendo.data.DataSource({
            ...
            transport: {
                read: {
                    url: ENV.vApi + "dealer/importHistoryRead",
                },
            ...
            filter: [{field: 'collection', operator: 'eq', value: "Dealer"}]
        }),
        detailInit: function (e) {
            $("<div/>").appendTo(e.detailCell).kendoGrid({
                dataSource: {
                    transport: {
                        read: {
                            url: ENV.vApi + "dealer/importHistoryDetail",
                        },
                        parameterMap: parameterMap
                    },
                    schema: {
...
var dealerHistory = function() {
    return {
        model: model,
        init: function () {
            kendo.bind("#dealer-history", kendo.observable(this.model));
            /*
             * Right Click Menu
             */
...
function cancelUpload(id) {
    $.ajax({
        url: ENV.vApi + `dealer/update_import_log/${id}`,
