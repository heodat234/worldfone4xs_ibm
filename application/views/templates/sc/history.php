...
<script>
    var model = kendo.observable({
        historyData: new kendo.data.DataSource({
            ...
            transport: {
                read: {
                    url: ENV.vApi + "sc/importHistoryRead",
                },
            ...
            filter: [{field: 'collection', operator: 'eq', value: "Sc"}]
        }),
        detailInit: function (e) {
            $("<div/>").appendTo(e.detailCell).kendoGrid({
                dataSource: {
                    transport: {
                        read: {
                            url: ENV.vApi + "sc/importHistoryDetail",
                        },
                        parameterMap: parameterMap
                    },
                    schema: {
    ...
    var scHistory = function() {
        return {
            model: model,
            init: function () {
                kendo.bind("#sc-history", kendo.observable(this.model));
                /*
                 * Right Click Menu
                 */
    ...
    $.ajax({
        url: ENV.vApi + `sc/update_import_log/${id}`,
