            ...
            <div class="row">
                <div id="popup-tabstrip" data-role="tabstrip" style="margin-top: 2px">
                    <ul>
                        ...
                        <li data-bind="click: openDatalibrary, invisible: isCallinglistCus">
...
window.popupObservable.assign({
    ...
    openDatalibrary: function(e) {
        var filter = JSON.stringify({
            logic: "and",
            filters: [
                {field: "id_no", operator: "eq", value: this.item.id_no}
            ]
        });
        var query = httpBuildQuery({filter: filter, omc: 1});
    ...
    isNewData: function(e) {
        if(!this.get('item.dl_assign') && !this.get('item.assigning') && this.get('item.id')) {
            return true
        }
        else return false
    },
