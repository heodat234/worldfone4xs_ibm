...
<script>
    var model = kendo.observable({
        historyData: new kendo.data.DataSource({
            ...
            transport: {
                read: {
                    url: ENV.vApi + "temporary_payment/importHistoryRead",
                },
            ...
            filter: [{field: 'collection', operator: 'eq', value: "LO_Temporary_payment"}]
        }),
        detailInit: function (e) {
            $("<div/>").appendTo(e.detailCell).kendoGrid({
                dataSource: {
                    transport: {
                        read: {
                            url: ENV.vApi + "temporary_payment/importHistoryDetail",
                        },
                        parameterMap: parameterMap
                    },
                    schema: {
...
var historyLog = function() {
    return {
        model: model,
        init: function () {
            kendo.bind("#history", kendo.observable(this.model));
            /*
             * Right Click Menu
             */
...
function cancelUpload(id) {
    $.ajax({
        url: ENV.vApi + `temporary_payment/update_import_log/${id}`,
