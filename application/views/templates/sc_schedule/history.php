...
<script>
    var model = kendo.observable({
        historyData: new kendo.data.DataSource({
            ...
            transport: {
                read: {
                    url: ENV.vApi + "sc_schedule/importHistoryRead",
                },
            ...
            filter: [{field: 'collection', operator: 'eq', value: "Sc_schedule"}]
        }),
...
var scScheduleHistory = function() {
    return {
        model: model,
        init: function () {
            kendo.bind("#sc-schedule-history", kendo.observable(this.model));
            /*
             * Right Click Menu
             */
...
scScheduleHistory.init();

function cancelUpload(id) {
    $.ajax({
        url: ENV.vApi + `sc_schedule/update_import_log/${id}`,
