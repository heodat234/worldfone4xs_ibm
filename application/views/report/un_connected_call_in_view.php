<div id="page-content">
	...
	<div class="container-fluid mvvm" style="padding-top: 20px; padding-bottom: 10px">
        ...
        <div class="row form-horizontal">
			<div class="form-group col-sm-4">
	            ...
	            <div class="col-xs-8">
	            	<input class="k-textbox" type="textbox" style="width: 100%" data-bind="value: customernumber"/>
	        	</div>
	        </div>
            <div class="form-group col-sm-8">
                <label class="control-label col-xs-2">@Queue@</label>
                <div class="col-xs-10">
                    <input data-role="dropdownlist"
                           data-value-primitive="true"
                           ...
                           data-bind="value: queue, source: queuesOption"/>
                </div>
            </div>
	    </div>
        <div class="row form-horizontal">
                        ...
                        data-columns="[
				            	{
				            	    field: 'date_call',
				            	    title: '@Date@',
				            	    template: data => gridDate(data.starttime, 'dd/MM/yyyy')
                                },
				            	{
				            	    field: 'time_call',
				            	    title: '@Time@',
				            	    template: data => gridDate(data.starttime, 'H:mm:ss')
                                },
				            	{
                                ...
                                },
				            	{
                                    ...
                                    title: '@Transfer to@',
                                    template: function(dataItem) {
                                        if(typeof dataItem.glide_extension !== 'undefined') {
                                            return gridArray(dataItem.glide_extension);
                                        }
                                        else {
                                            return dataItem.queue;
                                        }
                                    }
                                },
				            	{
				            	    field: 'waittimeinqueue',
				            	    title: '@Wait time in queue (second)@'
                                },
			            	]"
	...
	<script>
	    ...
	    var initReport = function() {
		    ...
		    var observable = window.observable = kendo.observable({
		        ...
		        dataReport: new kendo.data.DataSource({
                    ...
                    pageSize: 15,
		        	transport: {
		        		read: ENV.reportApi + "unconnectedcallin/read",
		        		parameterMap: parameterMap
		        	},
		        	// aggregate: [
        ...
        function exportGridExcel() {
			...
			$.ajax({
                url: ENV.reportApi + "unconnectedcallin/exportExcel",
