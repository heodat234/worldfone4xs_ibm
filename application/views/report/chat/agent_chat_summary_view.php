    ...
    <div align="center" style="align-items: center">
                ...
                <table class="text-center table table-borderless table-striped table-vcenter themed-background-white" style="border-radius: 5px;box-shadow: 0 4px 4px rgba(0,0,0,0.3), -4px 4px 0 rgba(0,0,0,0.3);">
                    <tbody id="missed-calls" >
                        ...
                        <tr> 
                            <td>Groups</td>
                            <td>
                                <select id="optional" onchange="checkline()" name="example-select[]" multiple="multiple" class="form-control" required>
                                </select>
                            </td>
                        </tr>
                        <tr>
                            <td>Agents</td> 
                            <td><input id="agentMultiSelect" name="Agent"  style="width: 250px" required />
                            </td>
                        </tr>
                        <tr>
                            <td>Start date</td>
        ...
        <div style="clear: both;">
            ...
            <div id="example" class="absConf" style="margin: 0 auto">
                ...
                <div id="grid-agent-availability" ></div>
            ...
            <script>
                ...
                $(document).ready(function () {
                    ...
                    $("#grid-agent-availability").kendoGrid({
                        toolbar: ["excel"],
                        excel: {
                            allPages: true,
                            fileName: "Agent Summary.xlsx"
                        },
                        pdf: {
                            allPages: true,
                            fileName: "Agent Summary.pdf"
                        },
                        dataSource: {
                            transport: {
                                read: {
                                    url: ENV.reportApi + "chat/agent_chat_summary/agentsum",
                                    type: "POST",
                                    dataType: 'json',
                                    contentType: "application/json",
                                    data: function () {
                                        return {from: $("#from").val(), to: $("#to").val(), agentList: agentList};
                                    }
                                },
                            ...
                            schema: {
                                // total_conversation_all: function (response) {
                                //     return response.total_conversation_all;
                                // },
                                // total_conversation_all: "total_conversation_all",
                                data: function (response) {
                                    var listWorkTime = [];
                                    $.each(response.data, function (key, val) {
                                        // listWorkTime.push(val.WorkTime);
                                        val.agent = val.agent;
                                    ...
                                    /*var maxWorkTime = secondsTimeSpanToHMS(findMaxWorkTime(listWorkTime));
                                    var totalAndAvgWorkTimeVar = totalAndAvgWorkTime(listWorkTime);
                                    $("#maxworktime-td").html(maxWorkTime);
                                    $("#avgworktime-td").html(secondsTimeSpanToHMS(totalAndAvgWorkTimeVar['avgWorkTime']));
                                    $("#totalworktime-td").html(secondsTimeSpanToHMS(totalAndAvgWorkTimeVar['totalWorkTime']));*/
                        ...
                        columns: [ 
...

                            {
                                ...
                                title:'% Total conversion',
                                filterable: false, 
                                template: '#= Math.round((total_conversation/total_conversation_all)*100) #%',
                            ...
                            },
                            {
                                ...
                                footerTemplate: '#= (data.fb_message.sum/data.total_conversation.sum)*100 #%'
                            },
                            {
                                field:'fb_message',
                                title:'Fb message',
                            ...
                            
                            {
                                ...
                                footerTemplate: '#= (data.fb_comment.sum/data.total_conversation.sum)*100 #%'
                            },
                            {
                                field:'fb_comment',
                                title:'Fb comment',
                            ...
                            
                            {
                                ...
                                footerTemplate: '#= (data.zalo_message.sum/data.total_conversation.sum)*100 #%'
                           ...
                           
                            {
                                ...
                                footerTemplate: '#= (data.livechat_message.sum/data.total_conversation.sum)*100 #%'
                            },
                            {
                                field:'livechat_message',
                                title:'Livechat message',
                       ...
                       /* [
                        {field: "userextension", title: "Agent", width: "110px", filterable: {multi: true}, template: '<a href="javascript:void(0);" onclick="redirectToDetail(#= userextension #);">#= userextension #</a>', footerTemplate: 'Total'},
                        {field: "sessiontime", title: "Session Time", width: "110px", filterable: false, footerTemplate: '#= secondsTimeSpanToHMS(data.sessiontimeCal.sum) #'},
                        {field: "pausetime", title: 'Pause Time', width: "110px", filterable: false, footerTemplate: '#= secondsTimeSpanToHMS(data.pausetimeCal.sum) #'},
                        {field: "WorkTime", title: 'Work Time', width: "110px", filterable: false, footerTemplate: '#= secondsTimeSpanToHMS(data.WorkTimeCal.sum) #'},
                        {field: "idletime", title: 'IDLE Time', width: "110px", filterable: false, footerTemplate: '#= secondsTimeSpanToHMS(data.idletimeCal.sum) #'},
                        {field: "calltime", title: 'Call Time', width: "110px", filterable: false, footerTemplate: '#= secondsTimeSpanToHMS(data.calltimecal.sum) #'},
                        ]*/
                    });
});
            ...
            function searchByDate(e) {
                ...
                var agentListTemp = $('#agentMultiSelect').data("kendoMultiSelectBox").value();
                if(agentListTemp) {
                    agentListTemp = agentListTemp.split(",");
                }
                else {
                    $.bootstrapGrowl('<h4> vui lòng chọn agent </h4> ', {
                        type: "danger",
                ...
                agentList = agentListTemp.map(function(value, index) {
                    /*return {extension:  value}//*/return value
                });
                // var groupListTemp = $('#optional').data("kendoMultiSelectBox").value();
              // console.log(queueText);
              $("#numberOfAgent").html(agentListTemp.length);
              $("#startdate-td").html($("#from").val());
              $("#enddate-td").html($("#to").val());
              datediff(parseDate($("#from").val()), parseDate($("#to").val()));
              $("#grid-agent-availability").data("kendoGrid").dataSource.read();
              $("#grid-group-availability").data("kendoGrid").dataSource.read();
              $("#grid-date-availability").data("kendoGrid").dataSource.read();
          }
        ...
        function checkline()
        {
            ...
            $.ajax({
                type: 'post',
                dataType: "json",
                url: ENV.reportApi + "chat/agent_chat_summary/ListAgent",
                data: {
                    data: data
                },
