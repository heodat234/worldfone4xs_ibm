<div id="page-content">
	...
	<script id="event-template" type="text/x-kendo-template">
	    <i class="#if(typeof status != 'undefined'){##: status.iconClass ##}#" style="vertical-align: -1px"></i>
	    <span>#: title #</span>
	</script>
	        	...
	        	schema: {
	        		...
	        		parse: function(response) {
	        			response.data.map(doc => {
	        				doc.image = "";
	        				doc.title = doc.status.text + (doc.substatus ? " (" + doc.substatus + ")" : "");
	        				doc.starttime = new Date(doc.starttime * 1000);
	        				doc.endtime = new Date(doc.endtime * 1000);
	        			})
	        			return response;
	        		},
